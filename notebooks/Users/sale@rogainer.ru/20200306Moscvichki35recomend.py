...
# MAGIC mkdir data
# MAGIC 
# MAGIC ./azcopy copy "https://bricksresult.blob.core.windows.net/data/moscviski35/full/*?st=2020-03-04T06%3A58%3A48Z&se=2020-12-05T06%3A58%3A00Z&sp=racwdl&sv=2018-03-28&sr=c&sig=LFmu8D%2FVsGZfEj%2F45W6nldF2lvq2mQr9kFl6AblqUN8%3D" data/ 

# COMMAND ----------

# MAGIC %sh
# MAGIC ./azcopy copy "https://bricksresult.blob.core.windows.net/data/client/client.parquet?st=2020-03-04T06%3A58%3A48Z&se=2020-12-05T06%3A58%3A00Z&sp=racwdl&sv=2018-03-28&sr=c&sig=LFmu8D%2FVsGZfEj%2F45W6nldF2lvq2mQr9kFl6AblqUN8%3D" data/client.parquet  
...
baught = list(set(purchases[purchases.client_id == 'D0407257304'].product_id)) 
...
rec = recomendations['D0407257304']
...
for js in tqdm(valid_data):
    row_sparse = make_coo_row(js["transaction_history"], product_encoder).tocsr()
    raw_recs = model.recommend(0, row_sparse, N=30, filter_already_liked_items=True, recalculate_user=True)
    recommended_items = product_encoder.toPid([x[0] for x in raw_recs])
    recomendations[js['client_id']] = recommended_items
    gt_items = js["target"][0]["product_ids"]
