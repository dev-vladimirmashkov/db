...
# если нужно глазами просмотреть прогноз - расскоментить, выведет все строки
...
# norm_res = pd.DataFrame(columns = ['ITEMID','forecast','date'], index = range(0,len(pred_dates)*8600))
# for index , content in forecasting.iterrows():
#     helper = pd.DataFrame(columns = ['ITEMID','forecast','date'], index = range(0,len(pred_dates)))
#     for counter, i in enumerate(content['forecast']):
#       helper.loc[counter,'ITEMID'] = content['ITEMID']
#       helper.loc[counter,'forecast'] = i
#       helper.loc[counter,'date'] = pred_dates[counter]
#     norm_res = pd.concat((helper.dropna(),norm_res))
#     norm_res = norm_res.reset_index(drop=True).sort_values(by='ITEMID').dropna()
#     helper = pd.DataFrame()
# forecasting = norm_res.copy().sort_index()